...
exports.types = {
  ...
  INTERNAL: 31,
  DATA: 32,
  // Private regtest-only proof-of-concept assignment.
  EXPERIMENTAL_HNSR: common.EXPERIMENTAL_HNSR
};
...
exports.typesByVal[exports.types.EXPERIMENTAL_HNSR] = 'EXPERIMENTAL_HNSR';
...
/**
 * Experimental HNSR Packet
 * @extends Packet
 * @property {Number} version
 * @property {Number} opcode
 * @property {Number} flags
 * @property {Buffer} contextID
 * @property {Buffer} body
 */

class HNSRPacket extends Packet {
  /**
   * Create a regtest-only HNSR packet.
   * @constructor
   * @param {Number?} version
   * @param {Number?} opcode
   * @param {Buffer?} contextID
   * @param {Buffer?} body
   */

  constructor(version, opcode, contextID, body) {
    super();

    this.type = exports.types.EXPERIMENTAL_HNSR;
    this.version = version != null ? version : common.hnsr.VERSION;
    this.opcode = opcode != null ? opcode : 0;
    this.flags = 0;
    this.contextID = contextID || Buffer.alloc(8);
    this.body = body || DUMMY;
  }

  getSize() {
    return 12 + this.body.length;
  }

  write(bw) {
    assert((this.version & 0xff) === this.version);
    assert((this.opcode & 0xff) === this.opcode);
    assert((this.flags & 0xffff) === this.flags);
    assert(this.flags === 0);
    assert(Buffer.isBuffer(this.contextID));
    assert(this.contextID.length === 8);
    assert(Buffer.isBuffer(this.body));
    assert(this.body.length <= common.hnsr.MAX_PACKET_SIZE - 12);

    bw.writeU8(this.version);
    bw.writeU8(this.opcode);
    bw.writeU16(this.flags);
    bw.writeBytes(this.contextID);
  ...
  read(br) {
    if (br.left() < 12)
      throw new Error('Truncated HNSR envelope.');

    this.version = br.readU8();
    this.opcode = br.readU8();
    this.flags = br.readU16();
    this.contextID = br.readBytes(8);

    if (this.version !== common.hnsr.VERSION)
      throw new Error('Unknown HNSR version.');

    if (this.flags !== 0)
      throw new Error('Reserved HNSR flags are set.');

    if (this.opcode > 20)
      throw new Error('Unknown HNSR opcode.');

    if (isZeroID(this.contextID) && this.opcode !== 6)
      throw new Error('HNSR context ID is zero.');
...
exports.decode = function decode(type, data) {
  switch (type) {
    ...
    case types.EXPERIMENTAL_HNSR:
      return HNSRPacket.decode(data);
    default:
