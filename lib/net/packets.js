...
exports.types = {
  ...
  INTERNAL: 31,
  DATA: 32,
  // Private experimental assignments.
  EXPERIMENTAL_GET_DNS_RELAY: common.EXPERIMENTAL_GET_DNS_RELAY,
  EXPERIMENTAL_DNS_RELAY: common.EXPERIMENTAL_DNS_RELAY,
  EXPERIMENTAL_ODOH: common.EXPERIMENTAL_ODOH
};
...
exports.typesByVal[exports.types.EXPERIMENTAL_GET_DNS_RELAY] =
  'EXPERIMENTAL_GET_DNS_RELAY';
exports.typesByVal[exports.types.EXPERIMENTAL_DNS_RELAY] =
  'EXPERIMENTAL_DNS_RELAY';
exports.typesByVal[exports.types.EXPERIMENTAL_ODOH] = 'EXPERIMENTAL_ODOH';
...
/**
 * Experimental Get DNS Relay Packet
 * @extends Packet
 * @property {Buffer} requestID
 * @property {Buffer} query
 */

class GetDNSRelayPacket extends Packet {
  /**
   * Create an experimental `getdnsrelay` packet.
   * @constructor
  ...
  constructor(requestID, query) {
    ...
    this.type = exports.types.EXPERIMENTAL_GET_DNS_RELAY;
    this.requestID = requestID || Buffer.alloc(common.dnsRelay.REQUEST_ID_SIZE);
    this.query = query || DUMMY;
  }
  ...
  getSize() {
    return common.dnsRelay.REQUEST_ID_SIZE + 2 + this.query.length;
  }
  ...
  write(bw) {
    assert(Buffer.isBuffer(this.requestID));
    assert(this.requestID.length === common.dnsRelay.REQUEST_ID_SIZE);
    assert(!isZeroID(this.requestID));
    assert(Buffer.isBuffer(this.query));
    assert(this.query.length > 0);
    assert(this.query.length <= common.dnsRelay.MAX_QUERY_SIZE);

    bw.writeBytes(this.requestID);
    bw.writeU16(this.query.length);
    bw.writeBytes(this.query);
   ...
   * Inject properties from buffer reader.
   * @param {bio.BufferReader} br
   * @returns {GetDNSRelayPacket}
   */

  read(br) {
    this.requestID = br.readBytes(common.dnsRelay.REQUEST_ID_SIZE);

    if (isZeroID(this.requestID))
      throw new Error('DNS relay request ID is zero.');

    const size = br.readU16();

    if (size === 0 || size > common.dnsRelay.MAX_QUERY_SIZE)
      throw new Error('DNS relay query exceeds size limits.');

    if (br.left() !== size)
      throw new Error('Invalid DNS relay query length.');

    this.query = br.readBytes(size);
...
class DNSRelayPacket extends Packet {
  /**
   * Create an experimental `dnsrelay` packet.
   * @constructor
   * @param {Buffer?} [requestID]
   * @param {Number?} [status]
  ...
  constructor(requestID, status, response) {
    super();

    this.type = exports.types.EXPERIMENTAL_DNS_RELAY;
    this.requestID = requestID || Buffer.alloc(common.dnsRelay.REQUEST_ID_SIZE);
    this.status = status != null ? status : common.dnsRelayStatus.OK;
    this.response = response || DUMMY;
  }
  ...
  getSize() {
    return common.dnsRelay.REQUEST_ID_SIZE + 3 + this.response.length;
  }
  ...
  write(bw) {
    ...
    assert(this.requestID.length === common.dnsRelay.REQUEST_ID_SIZE);
    assert(!isZeroID(this.requestID));
    assert(isU8(this.status));
    assert(Buffer.isBuffer(this.response));
    assert(this.response.length <= common.dnsRelay.MAX_RESPONSE_SIZE);

    if (this.status === common.dnsRelayStatus.OK)
    ...
    bw.writeU16(this.response.length);
    bw.writeBytes(this.response);
  ...
  read(br) {
    ...
    if (br.left() !== size)
      throw new Error('Invalid DNS relay response length.');

    if (this.status === common.dnsRelayStatus.OK && size === 0)
      throw new Error('Successful DNS relay response is empty.');

    if (this.status !== common.dnsRelayStatus.OK && size !== 0)
      throw new Error('Error DNS relay response has a body.');

    this.response = br.readBytes(size);
...
class ODNSPacket extends Packet {
  /**
   * Create an experimental `odns` packet.
   * @constructor
   * @param {Number?} [opcode]
   * @param {Buffer?} [requestID]
   * @param {Buffer?} [body]
  ...
  constructor(opcode, requestID, body) {
    super();

    this.type = exports.types.EXPERIMENTAL_ODOH;
    this.version = common.odoh.VERSION;
    this.opcode = opcode != null ? opcode : 0;
    this.flags = 0;
    this.requestID = requestID || Buffer.alloc(common.odoh.REQUEST_ID_SIZE);
    this.body = body || DUMMY;
  }

  getSize() {
    return common.odoh.ENVELOPE_SIZE + this.body.length;
  }

  write(bw) {
    assert(this.version === common.odoh.VERSION);
    assert(isU8(this.opcode));
    assert(this.opcode <= common.odohOpcodes.ERROR);
    assert(this.flags === 0);
    assert(Buffer.isBuffer(this.requestID));
    assert(this.requestID.length === common.odoh.REQUEST_ID_SIZE);
    assert(!isZeroID(this.requestID));
    assert(Buffer.isBuffer(this.body));
    assert(this.getSize() <= common.odoh.MAX_PACKET_SIZE);

    bw.writeU8(this.version);
    bw.writeU8(this.opcode);
    bw.writeU16(this.flags);
    bw.writeBytes(this.requestID);
    bw.writeBytes(this.body);

    return bw;
  }

  read(br) {
    if (br.left() < common.odoh.ENVELOPE_SIZE)
      throw new Error('Truncated P2P ODoH envelope.');

    this.version = br.readU8();
    this.opcode = br.readU8();
    this.flags = br.readU16();
    this.requestID = br.readBytes(common.odoh.REQUEST_ID_SIZE);

    if (this.version !== common.odoh.VERSION)
      throw new Error('Unsupported P2P ODoH envelope version.');

    if (this.opcode > common.odohOpcodes.ERROR)
      throw new Error('Unknown P2P ODoH opcode.');

    if (this.flags !== 0)
      throw new Error('Reserved P2P ODoH flags are nonzero.');

    if (isZeroID(this.requestID))
      throw new Error('P2P ODoH request ID is zero.');

    this.body = br.readBytes(br.left());
...
exports.decode = function decode(type, data) {
  switch (type) {
    ...
    case types.EXPERIMENTAL_GET_DNS_RELAY:
      return GetDNSRelayPacket.decode(data);
    case types.EXPERIMENTAL_DNS_RELAY:
      return DNSRelayPacket.decode(data);
    case types.EXPERIMENTAL_ODOH:
      return ODNSPacket.decode(data);
    default:
...
/**
 * Test whether an experimental request identifier is zero.
 * @param {Buffer} id
 * @returns {Boolean}
 */
...
/**
 * Test whether a value is an unsigned byte.
 * @param {Number} value
 * @returns {Boolean}
 */
...
exports.GetDNSRelayPacket = GetDNSRelayPacket;
exports.DNSRelayPacket = DNSRelayPacket;
exports.ODNSPacket = ODNSPacket;
