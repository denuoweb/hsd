...
class Pool extends EventEmitter {
  ...
  constructor(options) {
    ...
    this.dnsRelay = this.options.dnsRelay;
    this.odoh = this.options.odoh;
  ...
  /**
   * Attach an optional DNS relay service before opening the pool.
   * @param {Object} service
   * @returns {Pool}
   */

  setDNSRelay(service) {
    assert(!this.opened, 'Cannot attach DNS relay to an open pool.');
    assert(service && typeof service.isReady === 'function');
    assert(typeof service.handle === 'function');
    assert(typeof service.cancelPeer === 'function');

    this.dnsRelay = service;
    this.options.dnsRelay = service;

    return this;
  }

  /**
   * Attach an optional P2P ODoH service before opening the pool.
   * @param {Object} service
   * @returns {Pool}
   */

  setODoH(service) {
    assert(!this.opened, 'Cannot attach ODoH to an open pool.');
    assert(service && typeof service.isReady === 'function');
    assert(typeof service.handle === 'function');
    assert(typeof service.cancelPeer === 'function');

    this.odoh = service;
    this.options.odoh = service;
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.AIRDROP:
        ...
        break;
      case packetTypes.EXPERIMENTAL_GET_DNS_RELAY:
        await this.handleGetDNSRelay(peer, packet);
        break;
      case packetTypes.EXPERIMENTAL_DNS_RELAY:
        await this.handleDNSRelay(peer, packet);
        break;
      case packetTypes.EXPERIMENTAL_ODOH:
        await this.handleODoH(peer, packet);
        break;
  ...
  async handleClose(peer, connected) {
    ...
    if (this.dnsRelay)
      this.dnsRelay.cancelPeer(peer);

    if (this.odoh)
      this.odoh.cancelPeer(peer);
  ...
  /**
   * Handle experimental `getdnsrelay` packet.
   *
   * Backend work is intentionally detached from Peer's packet lock so one
   * recursive lookup does not stall unrelated traffic on the connection.
   * @method
   * @private
   * @param {Peer} peer
  ...
  async handleGetDNSRelay(peer, packet) {
    if (!this.dnsRelay)
      return;

    this.dnsRelay.handle(peer, packet).catch((err) => {
      this.emit('error', err);
    });
  }

  /**
   * Handle an unsolicited experimental `dnsrelay` packet.
   * @method
   * @private
   * @param {Peer} peer
   * @param {DNSRelayPacket} packet
   */

  async handleDNSRelay(peer, packet) {
    this.logger.debug(
      'Unsolicited DNS relay response (status=%d) (%s).',
      packet.status,
      peer.hostname());
  }

  /**
   * Handle an experimental P2P ODoH packet without retaining Peer's packet
   * lock across HPKE, recursion, or a target round trip.
   * @method
   * @private
   * @param {Peer} peer
   * @param {ODNSPacket} packet
   */

  async handleODoH(peer, packet) {
    if (!this.odoh)
      return;

    this.odoh.handle(peer, packet).catch((err) => {
      this.emit('error', err);
    });
  }
...
class PoolOptions {
  ...
  constructor(options) {
    ...
    this.getServices = this._getServices.bind(this);
    this.proxy = null;
    this.onion = false;
  ...
  fromOptions(options) {
    ...
    if (options.dnsRelay != null) {
      assert(typeof options.dnsRelay === 'object');
      assert(typeof options.dnsRelay.isReady === 'function');
      assert(typeof options.dnsRelay.handle === 'function');
      assert(typeof options.dnsRelay.cancelPeer === 'function');
      this.dnsRelay = options.dnsRelay;
    }

    if (options.odoh != null) {
      assert(typeof options.odoh === 'object');
      assert(typeof options.odoh.isReady === 'function');
      assert(typeof options.odoh.handle === 'function');
      assert(typeof options.odoh.cancelPeer === 'function');
      this.odoh = options.odoh;
    }
  ...
  /**
   * Get services for the current version handshake.
   * @private
   * @returns {Number}
   */

  _getServices() {
    let bits = this.services
      & ~common.EXPERIMENTAL_DNS_RELAY_SERVICE
      & ~common.EXPERIMENTAL_ODOH_SERVICE;

    if (this.dnsRelay && this.dnsRelay.isReady())
      bits |= common.EXPERIMENTAL_DNS_RELAY_SERVICE;

    if (this.odoh && this.odoh.isReady())
      bits |= common.EXPERIMENTAL_ODOH_SERVICE;
