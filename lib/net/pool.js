...
class Pool extends EventEmitter {
  ...
  /**
   * Attach the optional regtest HNSR service before the pool opens.
   * @param {HNSRService} service
   * @returns {Pool}
   */

  setHNSR(service) {
    assert(!this.opened, 'Cannot attach HNSR to an open pool.');
    assert(service && typeof service.handle === 'function');
    assert(typeof service.cancelPeer === 'function');

    this.hnsr = service;
    service.pool = this;
    service.on('circuit', (socket, info) => {
      if (info.profile !== 1)
        return;

      try {
        const peer = this.addHNSRInbound(socket, info);
        service.emit('virtual peer', peer, info);
      } catch (e) {
        socket.destroy();
        this.emit('error', e);
      }
  ...
  /**
   * Find an authenticated live peer by static Brontide key.
   * @param {Buffer} key
   * @returns {Peer?}
   */
  ...
  findHNSRPeer(key) {
    ...
    for (let peer = this.peers.head(); peer; peer = peer.next) {
      if (peer.destroyed || peer.hnsrVirtual)
        continue;

      if (peer.address.key.equals(key)
  ...
  async _connectHNSRAddress(addr, requiredServices) {
    let peer = this.findHNSRPeer(addr.key);

    if (peer && peer.handshake) {
      if ((peer.services & requiredServices) !== requiredServices)
        throw new Error('Connected peer lacks the required HNSR role.');
      return peer;
    }

    peer = this.createOutbound(addr);
    peer.hnsrControl = true;
    this.peers.add(peer);
    this.emit('peer', peer);
    ...
    await new Promise((resolve, reject) => {
      ...
      onClose = () => {
        cleanup();
        reject(new Error('HNSR discovered peer disconnected.'));
      };
      timer = setTimeout(() => {
        cleanup();
        peer.destroy();
        reject(new Error('HNSR discovered peer handshake timed out.'));
      }, 10000);
    ...
    if ((peer.services & requiredServices) !== requiredServices) {
      peer.destroy();
      throw new Error('Discovered peer lacks the required HNSR role.');
    }

    return peer;
  }

  async connectHNSRContact(contact) {
    const addr = contact.toAddress(this.network);
    return this._connectHNSRAddress(
      addr,
      common.EXPERIMENTAL_HNSR_RENDEZVOUS_SERVICE);
  }

  async connectHNSRTicket(ticket) {
    const peer = this.findHNSRPeer(ticket.relayKey);

    if (peer && peer.handshake
        && (peer.services & common.EXPERIMENTAL_HNSR_RELAY_SERVICE) !== 0) {
      return peer;
    }

    const addr = NetAddress.fromHost(
      IP.toString(ticket.host),
      ticket.port,
      ticket.relayKey,
      this.network);
    addr.services = common.services.NETWORK
  ...
  addHNSRInbound(socket, info) {
    if (!this.opened)
      throw new Error('Cannot add an HNSR peer to a closed pool.');

    const peer = this.createInbound(socket, true);
    peer.hnsrVirtual = true;
    peer.hnsrInfo = info;
    this.peers.add(peer);
    return peer;
  }

  async addHNSROutbound(socket, endpointKey, info) {
    if (!this.opened)
      throw new Error('Cannot add an HNSR peer to a closed pool.');
    ...
    await new Promise((resolve, reject) => {
      ...
      onClose = () => {
        cleanup();
        reject(new Error('Inner HNSR peer disconnected.'));
      };
      timer = setTimeout(() => {
        cleanup();
        peer.destroy();
        reject(new Error('Inner HNSR peer handshake timed out.'));
      }, 10000);
  ...
  async handlePacket(peer, packet) {
    switch (packet.type) {
      ...
      case packetTypes.AIRDROP:
        ...
        break;
      case packetTypes.EXPERIMENTAL_HNSR:
        await this.handleHNSR(peer, packet);
        break;
  ...
  async handleConnect(peer) {
    ...
    if (peer.outbound && !peer.hnsrVirtual)
  ...
  /**
   * Handle one experimental HNSR envelope.
   * @method
   * @private
   * @param {Peer} peer
   * @param {HNSRPacket} packet
   */

  async handleHNSR(peer, packet) {
    if (!this.hnsr || !this.hnsr.isReady()) {
      this.logger.debug('Ignoring HNSR packet from %s.', peer.hostname());
      return;
    }

    await this.hnsr.handle(peer, packet);
  }
