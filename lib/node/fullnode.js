...
const {HNSRService} = require('../net/hnsr');
...
class FullNode extends Node {
  ...
  constructor(options) {
    ...
    const hnsrRendezvous = this.config.bool(
      'experimental-hnsr-rendezvous');
    const hnsrRelay = this.config.bool('experimental-hnsr-relay');
    const hnsrEndpoint = this.config.bool('experimental-hnsr-endpoint');
    const hnsrWeb = this.config.bool('experimental-hnsr-web');
    const hnsrTestnet = this.config.bool('experimental-hnsr-testnet');
    const hnsrEnabled = this.config.bool('experimental-hnsr')
      || hnsrRendezvous
      || hnsrRelay
      || hnsrEndpoint
      || hnsrWeb;
    ...
    if (hnsrEnabled
        ...
        && !(this.network.type === 'testnet' && hnsrTestnet)) {
      throw new Error(
        'Private HNSR is regtest-only unless testnet is explicitly '
        + 'acknowledged.');
    }

    if (hnsrTestnet && this.network.type !== 'testnet') {
      throw new Error(
        'The HNSR testnet acknowledgement is valid only on testnet.');
    }

    if (hnsrRelay && !this.pool.options.listen) {
      throw new Error(
        'The HNSR relay role requires an ordinary listening peer.');
    }

    const hnsrPublicHost = IP.toBuffer(this.pool.options.publicHost);

    if (hnsrEnabled
        && this.network.type === 'testnet'
        && (hnsrRelay || hnsrRendezvous)
        && !IP.isRoutable(hnsrPublicHost)) {
      throw new Error(
        'HNSR testnet relay and rendezvous roles require a public host.');
    }

    this.hnsr = new HNSRService({
      enabled: hnsrEnabled,
      rendezvous: hnsrRendezvous,
      relay: hnsrRelay,
      endpoint: hnsrEndpoint,
      web: hnsrWeb,
      identityKey: this.identityKey,
      network: this.network,
      chain: this.chain,
      ...
      memory: this.memory,
      prefix: this.config.prefix,
      persist: this.config.bool('experimental-hnsr-persist', true),
      relayHost: IP.isRoutable(hnsrPublicHost)
        ? hnsrPublicHost
        : undefined,
      relayPort: this.pool.options.publicBrontidePort,
      timeout: this.config.uint(
        'experimental-hnsr-timeout',
        netCommon.hnsr.DEFAULT_TIMEOUT)
    });

    this.pool.setHNSR(this.hnsr);

    const hnsrServices = this.hnsr.services();
    this.pool.options.services |= hnsrServices;
    this.pool.options.services >>>= 0;
    this.pool.hosts.options.services = this.pool.options.services;
    this.pool.hosts.address.services = this.pool.options.services;
    this.pool.hosts.brontide.services = this.pool.options.services;
  ...
  init() {
    ...
    this.hnsr.on('error', err => this.error(err));
