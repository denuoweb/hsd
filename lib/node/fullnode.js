...
} = require('../net/dnsrelay');
const {ODoHService} = require('../net/odoh');
const netCommon = require('../net/common');
...
class FullNode extends Node {
  ...
  constructor(options) {
    ...
    const relayEnabled = this.config.bool('experimental-dns-relay');
    const allowPrivateRelayAuthorities = this.config.bool(
      'experimental-dns-relay-allow-private-authorities');

    if (allowPrivateRelayAuthorities && this.network.type !== 'regtest') {
      throw new Error(
        'Private DNS relay authorities are permitted only on regtest.');
    }

    if (allowPrivateRelayAuthorities && !relayEnabled) {
      throw new Error(
        'Private DNS relay authorities require experimental DNS relay.');
    }

    const relayBackend = this.rs
      ? new RecursiveDNSRelayBackend(this.rs, null, {
        allowPrivateAuthorities: allowPrivateRelayAuthorities
      })
      : null;

    this.dnsRelay = new DNSRelayService({
      enabled: relayEnabled,
      chain: this.chain,
      rootServer: this.ns,
      backend: relayBackend,
      logger: this.logger,
      timeout: this.config.uint(
        'experimental-dns-relay-timeout',
        netCommon.dnsRelay.DEFAULT_TIMEOUT),
      rate: this.config.uint(
        'experimental-dns-relay-rate',
        netCommon.dnsRelay.DEFAULT_RATE),
      burst: this.config.uint(
        'experimental-dns-relay-burst',
        netCommon.dnsRelay.DEFAULT_BURST),
      globalRate: this.config.uint(
        'experimental-dns-relay-global-rate',
        netCommon.dnsRelay.DEFAULT_GLOBAL_RATE),
      globalBurst: this.config.uint(
        'experimental-dns-relay-global-burst',
        netCommon.dnsRelay.DEFAULT_GLOBAL_BURST),
      maxPeerInflight: this.config.uint(
        'experimental-dns-relay-peer-inflight',
        netCommon.dnsRelay.DEFAULT_PEER_INFLIGHT),
      maxGlobalInflight: this.config.uint(
        'experimental-dns-relay-global-inflight',
        netCommon.dnsRelay.DEFAULT_GLOBAL_INFLIGHT)
    });

    this.pool.setDNSRelay(this.dnsRelay);

    const odohProxy = this.config.bool('experimental-odoh-proxy');
    const odohTarget = this.config.bool('experimental-odoh-target');
    const allowPrivateODoHTargets = this.config.bool(
      'experimental-odoh-allow-private-targets');

    if (odohTarget && !relayEnabled)
      throw new Error('ODoH target requires experimental DNS relay.');

    if (allowPrivateODoHTargets && this.network.type !== 'regtest') {
      throw new Error('Private ODoH targets are permitted only on regtest.');
    }

    const defaultTargetHost = this.network.type === 'regtest'
      ? '127.0.0.1'
      : null;

    this.odoh = new ODoHService({
      pool: this.pool,
      network: this.network,
      identityKey: this.identityKey,
      dnsRelay: this.dnsRelay,
      logger: this.logger,
      proxy: odohProxy,
      target: odohTarget,
      allowPrivate: allowPrivateODoHTargets,
      instrumentation: this.config.bool(
        'experimental-odoh-instrumentation'),
      targetHost: this.config.str(
        'experimental-odoh-target-host', defaultTargetHost),
      targetPort: this.config.uint(
        'experimental-odoh-target-port', this.pool.options.brontidePort),
      timeout: this.config.uint(
        'experimental-odoh-timeout', netCommon.odoh.DEFAULT_TIMEOUT),
      maxLive: this.config.uint(
        'experimental-odoh-max-live', netCommon.odoh.MAX_LIVE_GLOBAL),
      rotationInterval: this.config.uint(
        'experimental-odoh-key-rotation',
        netCommon.odoh.DEFAULT_KEY_ROTATION),
      keyOverlap: this.config.uint(
        'experimental-odoh-key-overlap',
        netCommon.odoh.DEFAULT_KEY_OVERLAP)
    });

    this.pool.setODoH(this.odoh);
  ...
  async open() {
    ...
    await this.dnsRelay.open();
    await this.odoh.open();
  ...
  async close() {
    ...
    await this.odoh.close();
    await this.dnsRelay.close();
